...
impl BinarySplit for KDTreeBranch {
    ...
    fn dist(&self, point: &[f64]) -> f64 {
        let mut d = 0.;
        for ((&p, &mi), &ma) in point.iter().zip(self.min.iter()).zip(self.max.iter()) {
            if p < mi {
                d += (mi - p) * (mi - p);
            } else if ma < p {
                d += (ma - p) * (ma - p);
...
impl<B: BinarySplit> BinaryTree<B> {
    ...
    fn select_split(
        ...
        loop {
            ...
            let split = unsafe { dmin.data().get_unchecked(dim) + d / 2.0 };
            ...
            loop {
                ...
                match *current {
                    ...
                    Node::Branch(ref b) => {
                        ...
                        let (close, far) = unsafe { b.maybe_close(point) };
...
mod tests {
    ...
    use super::{max, min, BallTree, KDTree};
    use linalg::{BaseMatrix, Matrix, Vector};

    use super::{BinarySplit, Leaf, Node};
    ...
    fn test_kdtree_search() {
        ...
        assert_eq!(
            kn.pairs,
            vec![(0, (2.0f64 * 2.0f64 + 2.9f64 * 2.9f64).sqrt())]
        );
        ...
        assert_eq!(
            kn.pairs,
            vec![
                (0, (2.0f64 * 2.0f64 + 2.9f64 * 2.9f64).sqrt()),
                (
                    4,
                    (3.0f64 * 3.0f64 + (4.9f64 - 3.0f64) * (4.9f64 - 3.0f64)).sqrt()
                )
            ]
        );
        // search tree
    ...
    fn test_kdtree_dim_selection() {
        let m = Matrix::new(5, 2, vec![1., 2., 3., 0., 2., 10., 3., 6., 1., 3.]);
        let mut tree = KDTree::new(3);
        tree.build(m);
    ...
    fn test_kdtree_dim_selection_biased() {
        let m = Matrix::new(5, 2, vec![1., 0., 3., 0., 2., 20., 3., 0., 1., 0.]);
        let mut tree = KDTree::new(3);
        tree.build(m);
    ...
    #[test]
    fn test_min_max() {
        let data = Matrix::new(3, 2, vec![1., 2., 2., 4., 3., 1.]);
